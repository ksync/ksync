...
func (e *execCmd) new() *cobra.Command {
	...
	flags.BoolP(
		...
		"t",
		false,
		"Allocate a pseudo tty to this session.")
	if err := e.BindFlag("tty"); err != nil {
		log.Fatal(err)
	}
...
func (e *execCmd) run(_ *cobra.Command, args []string) {
	// Default case: construct spec and run command
